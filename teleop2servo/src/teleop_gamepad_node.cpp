...
TeleopGamepadNode::TeleopGamepadNode(const rclcpp::NodeOptions& options)
    : Node("gamepad_teleop_node", options), teleop_publisher_(*this, TeleopDevice::GAMEPAD) {
...
void TeleopGamepadNode::setup_subscribers() {
  joy_sub_ = create_subscription<sensor_msgs::msg::Joy>(
      gamepad_config_.joy_topic, rclcpp::SensorDataQoS(),
      std::bind(&TeleopGamepadNode::joy_callback, this, std::placeholders::_1));
}

bool TeleopGamepadNode::button_pressed(const sensor_msgs::msg::Joy::SharedPtr& msg, Button button) const {
  const int index = static_cast<int>(button);
  return msg && static_cast<size_t>(index) < msg->buttons.size() && msg->buttons[index] != 0;
}

bool TeleopGamepadNode::rising_edge(const sensor_msgs::msg::Joy::SharedPtr& msg, Button button) const {
...
double TeleopGamepadNode::axis_value(const sensor_msgs::msg::Joy::SharedPtr& msg, Axis axis) const {
  const int index = static_cast<int>(axis);
  if (!msg || index < 0 || static_cast<size_t>(index) >= msg->axes.size())
    return 0.0;

  return static_cast<double>(msg->axes[index]);
}

bool TeleopGamepadNode::joy_in_use(const sensor_msgs::msg::Joy::SharedPtr& msg) const {
  if (!msg)
    return false;
...
bool TeleopGamepadNode::check_state_buttons(const sensor_msgs::msg::Joy::SharedPtr& msg) {
...
double TeleopGamepadNode::button_pair_direction(const sensor_msgs::msg::Joy::SharedPtr& msg,
...
std::pair<double, double> TeleopGamepadNode::axis_direction(const sensor_msgs::msg::Joy::SharedPtr& msg,
                                                            Axis x_axis,
                                                            Axis y_axis,
                                                            Button step_button,
...
void TeleopGamepadNode::create_cmd_joint(const sensor_msgs::msg::Joy::SharedPtr& msg,
  ...
  auto set_joint = [this, &cmd, &msg, speed_mode, scale](std::size_t i, Button positive, Button negative) {
    if (i >= cmd.joint_velocities.size())
      return;
...
void TeleopGamepadNode::create_cmd_twist(const sensor_msgs::msg::Joy::SharedPtr& msg,
  ...
  const auto [linear_y_dir, linear_x_dir] = axis_direction(msg, GamepadMapping::x_axis, GamepadMapping::y_axis,
                                                           GamepadMapping::linear_step_button, speed_mode);

  const double linear_z_dir =
      button_pair_direction(msg, GamepadMapping::z_positive, GamepadMapping::z_negative, speed_mode);

  const auto [angular_y_dir, angular_x_dir] = axis_direction(msg, GamepadMapping::roll_axis, GamepadMapping::pitch_axis,
                                                             GamepadMapping::angular_step_button, speed_mode);
